# learner/gmm_hmm.py
# ------------------------------------------------------------
# Baseline GMM-HMM (HMM-HMM)
# Works for 2D/3D; features same as your old baseline
# Uses 4-panel plot too (panel4 shows xi01-normalized curve)
# 并兼容 GoalHMM3D / plot4panel 的接口
# ------------------------------------------------------------
...
class GMMHMM:
    def __init__(
        ...
        GoalHMM3D._init_oracle_constraints(self)
    ...
    # ------------------------------------------------------------
    # 兼容 GoalHMM3D：给 plot4panel 用的可视化边界
    # ------------------------------------------------------------
    ...
    def fit(self, max_iter=30, verbose=True):
        ...
        for it in range(max_iter):
            ...
            try:
                d_est, v_est = self.estimate_constraint_thresholds(k_sigma=1.0)  # 或 2.0，看你 GoalHMM 那边用多少σ
