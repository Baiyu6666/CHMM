...
class SegCons:
    def __init__(
        ...
        if self.plot_every is not None and plt is None:
            print("[SegCons] matplotlib is not installed; plots will not be generated.")
        ...
        if self._has_learned:
            if torch is None:
                raise ImportError("learned_features requires torch, but torch is not installed.")
            params = []
    ...
    def _compute_all_features_raw(self, X):
        ...
        # --- 2) learnable features（由 SegCons 持有） ---
    ...
    def _mstep_update_learned_features(self, gammas):
        if not self._has_learned or self.g_optimizer is None or torch is None:
            return
        ...
        for it in range(max_iter):
            ...
            if self.plot_every is not None:
                if (it) % self.plot_every == 0 or it == max_iter - 1:
                    ...
                    #     print("[SegCons] _plot_learned_sine_xy skipped:", repr(e))
    ...
    def _plot_learned_sine_xy(self, gammas):
        ...
        for i, lf in enumerate(self.learned_features):
            ...
            fig = plt.gcf()
            save_figure(fig, learner_plot_dir(self) / f"learned_sine_state_{int(k)}.png")
