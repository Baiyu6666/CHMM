...
class PressSlideInsertEnv2D:
    def __init__(
        self,
        seg_lengths=(24, 12, 24, 18),
        start=(-1.4, 0.35),
        ...
        seg_length_jitter=(4, 2, 5, 3),
        seg_length_scale_range=(0.88, 1.15),
        dt: float = 0.7,
    ):
        ...
        self.dt = float(dt)
        self.eval_tag = "2DPressSlideInsert"
        self.n_segments = 4
    ...
    def _smooth_positive_trace(values: np.ndarray, kernel_size: int = 7) -> np.ndarray:
        ...
        return PressSlideInsertEnv2D._smooth_trace(vals, kernel_size=kernel_size)
    ...
    def _sample_segment_lengths(self, rng: np.random.RandomState):
        ...
        for base, jitter, min_len in zip(self.seg_lengths, self.seg_length_jitter, min_lengths):
            local_scale = global_scale * float(rng.uniform(0.94, 1.08))
    ...
    def generate_demo(self, seed: int):
        ...
        stage2_end_local = self.stage2_end + rng.randn(2) * self.stage_end_jitter[1]
        ...
        stage1_end_local[0] = float(np.clip(stage1_end_local[0], -0.34, -0.12))
        stage1_end_local[1] = float(np.clip(stage1_end_local[1], 0.012, 0.042))
        stage2_end_local[0] = float(np.clip(max(stage2_end_local[0], stage1_end_local[0] + 0.02), -0.26, -0.08))
        stage2_end_local[1] = float(np.clip(stage2_end_local[1], -0.010, 0.010))
        stage3_end_local[0] = float(np.clip(max(stage3_end_local[0], stage2_end_local[0] + 0.78), 0.64, 0.78))
        stage3_end_local[1] = float(np.clip(stage3_end_local[1], -0.010, 0.010))
        stage4_end_local[0] = float(np.clip(max(stage4_end_local[0], stage3_end_local[0] + 0.22), 0.96, 1.03))
        stage4_end_local[1] = float(np.clip(stage4_end_local[1], -0.008, 0.008))

        v1_demo = self.v1_target * rng.uniform(0.94, 1.06)
        v3_demo = self.v3_target * rng.uniform(1.04, 1.14)
        v4_demo = self.v4_target * rng.uniform(0.92, 1.08)
        ...
        seg2 = self._piecewise_segment(stage1_end_local, stage2_end_local, l2 + 1, endpoint=True)[:-1]
        u2 = np.linspace(0.0, 1.0, l2, endpoint=False)
        seg2[:, 0] += 0.012 * latents["style"] * np.sin(2.0 * np.pi * u2 + latents["phase"]) * np.sin(np.pi * u2)
        seg2[:, 1] += (
            0.008 * np.exp(-2.8 * u2) * np.sin(3.2 * np.pi * u2 + latents["phase"])
            - 0.003 * np.sin(np.pi * u2) ** 2
        )
        ...
        seg4[:, 1] += 0.3 * latents["surface_wobble"] * np.sin(2.0 * np.pi * u4 + latents["phase"]) * np.sin(np.pi * u4)

        seg1 = self._timewarp_decelerating_path(seg1, strength=0.85, rng=rng, floor=0.46)
        seg2 = self._timewarp_path(seg2, strength=0.14, rng=rng, cycles=1.8)
        seg3 = self._timewarp_path(seg3, strength=0.16, rng=rng, cycles=2.0)
        seg4 = self._timewarp_path(seg4, strength=0.12, rng=rng, cycles=1.6)
        ...
        theta_stage2_end = self.theta_stage2_end + self.theta_end_jitter[1] * rng.randn()
        ...
        theta3 = np.linspace(theta_stage2_end, theta_stage3_end, l3, endpoint=False)
        theta4 = np.linspace(theta_stage3_end, theta_stage4_end, l4, endpoint=True)
        ...
        pos[:, 0] += self._smooth_noise(rng, len(pos), 1.35 * self.noise_pos, kernel_size=11)
        pos[:, 1] += self._smooth_noise(rng, len(pos), 0.95 * self.noise_pos, kernel_size=9)
        ...
        theta += self._smooth_noise(rng, len(theta), 0.28 * self.noise_misc, kernel_size=11)
        theta += latents["theta_wobble"] * np.sin(np.linspace(0.0, 4.5 * np.pi, len(theta)) + latents["phase"]) * np.r_[
            np.linspace(0.3, 1.0, l1 + l2),
            np.linspace(1.0, 0.45, l3 + l4),
        ]
        theta[l1 + l2:] = np.clip(theta[l1 + l2:], -0.12, 0.12)
        for boundary in (l1 - 1, l1 + l2 - 1, l1 + l2 + l3 - 1):
        ...
        return np.asarray(pos, dtype=float), np.asarray(theta, dtype=float), np.asarray(labels, dtype=int), force
...
def load_2d_press_slide_insert(
    n_demos: int = 10,
    seed: int = 123,
    env_kwargs=None,
):
    ...
    env = PressSlideInsertEnv2D(**env_cfg)

    demos = []
    labels = []
    for i in range(n_demos):
        pos, theta, z, force = env.generate_demo(seed=seed + i)
    ...
    env.subgoal = np.mean([x[int(c[1]), :3] for x, c in zip(demos, cutpoints)], axis=0)
    env.goal = np.mean([x[-1, :3] for x in demos], axis=0)

    return TaskBundle(
        name="2DPressSlideInsert",
        demos=demos,
        env=env,
        true_taus=None,
        ...
        meta={"seed": seed, "cutpoints": [c.tolist() for c in cutpoints], "task_name": "2DPressSlideInsert"},
    )
