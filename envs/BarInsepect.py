...
class BarInsepectEnv:
    ...
    def __init__(
        ...
        if len(self.seg_lengths) != 4 or len(self.seg_length_jitter) != 4:
            raise ValueError("BarInsepect requires four segment lengths and four jitters.")
        if min(self.seg_lengths) < 4 or min(self.seg_length_jitter) < 0:
            raise ValueError("BarInsepect segment lengths must be >= 4 and jitters non-negative.")
        ...
        self._cached_bar_pose_traces = {}
        self._cached_obstacle_position_traces = {}
    ...
    @staticmethod
    def _trajectory_cache_key(trajectory):
        array = np.ascontiguousarray(np.asarray(trajectory, dtype=np.float64))
        return array.shape, array.tobytes()

    def register_bar_pose_trace(self, trajectory, bar_pose):
        """Associate an OptiTrack ``[x, y, z, qx, qy, qz, qw]`` trace with a demo."""
        demo = np.asarray(trajectory, dtype=float)
        poses = np.asarray(bar_pose, dtype=float)
        if poses.ndim != 2 or poses.shape != (len(demo), 7):
            raise ValueError(
                "A BarInsepect bar-pose trace must have shape (len(trajectory), 7)."
            )
        self._quat_normalize(poses[:, 3:7])
        self._cached_bar_pose_traces[self._trajectory_cache_key(demo)] = poses.copy()

    def register_obstacle_pose_trace(self, trajectory, obstacle_pose):
        """Associate synchronized ``baiyu_obs_ball`` positions with a demo.

        ``obstacle_pose`` may contain XYZ positions or full XYZ+quaternion poses.
        Orientation is intentionally ignored because the obstacle is modeled as
        an infinite cylinder parallel to the calibrated table normal.
        """
        demo = np.asarray(trajectory, dtype=float)
        poses = np.asarray(obstacle_pose, dtype=float)
        if poses.ndim != 2 or len(poses) != len(demo) or poses.shape[1] not in (3, 7):
            raise ValueError(
                "A BarInsepect obstacle-pose trace must have shape "
                "(len(trajectory), 3) or (len(trajectory), 7)."
            )
        positions = (
        ...
        if not np.all(np.isfinite(positions)):
            raise ValueError("BarInsepect obstacle positions must be finite.")
        self._cached_obstacle_position_traces[
            self._trajectory_cache_key(demo)
        ] = positions.copy()

    def _obstacle_center_trace(self, trajectory):
        demo = np.asarray(trajectory, dtype=float)
        positions = self._cached_obstacle_position_traces.get(
            self._trajectory_cache_key(demo)
        )
        if positions is not None:
            return positions
        return np.repeat(self.obstacle_center[None, :], len(demo), axis=0)

    def _bar_geometry_trace(self, trajectory):
        demo = np.asarray(trajectory, dtype=float)
        bar_pose = self._cached_bar_pose_traces.get(self._trajectory_cache_key(demo))
        if bar_pose is None:
    ...
    def get_feature_schema(self):
        return [
            ...
            {"id": 2, "column_idx": 2, "name": "bar_lateral_offset", "unit": "m",
             "description": "Signed in-table offset from the bar centerline; diagnostic and excluded from the default learner"},
    ...
    def get_observation_spec(self):
        return {
            ...
            "optitrack_obstacle_pose": {
                ...
                "registration_method": "register_obstacle_pose_trace",
            ...
            "optitrack_bar_pose": {
                ...
                "registration_method": "register_bar_pose_trace",
    ...
    def compute_observation(self, latent_rollout, scene):
        ...
        if bar_pose is not None:
            self.register_bar_pose_trace(trajectory, bar_pose)
        obstacle_pose = latent_rollout.get("obstacle_pose")
        if obstacle_pose is not None:
            self.register_obstacle_pose_trace(trajectory, obstacle_pose)
        return {
            "trajectory": trajectory,
            ...
            "features": self.compute_all_features_matrix(trajectory),
    ...
    def compute_all_features_matrix(self, traj, feat_ids=None):
        ...
        if trajectory.ndim != 2 or trajectory.shape[1] < 7:
            raise ValueError(
                "BarInsepect trajectories must contain [x, y, z, qx, qy, qz, qw]. "
        ...
        bar_reference_point, bar_axis, bar_lateral = self._bar_geometry_trace(trajectory)
        obstacle_center = self._obstacle_center_trace(trajectory)
        ...
        if not np.all(np.isfinite(features)):
            raise ValueError("BarInsepect feature extraction produced non-finite values.")
        if feat_ids is None:
            return features
    ...
    **extra_env_kwargs,
    ...
    if processed_demo_path is not None:
        ...
        with np.load(data_path, allow_pickle=False) as archive:
            ...
            if missing:
                raise ValueError(
                    f"Processed BarInsepect data {data_path} is missing keys: {missing}"
                )
        ...
        if feature_names != expected_names:
            raise ValueError(
                "Processed BarInsepect feature order does not match the environment schema: "
...
__all__ = ["BarInsepectEnv", "load_BarInsepect"]
