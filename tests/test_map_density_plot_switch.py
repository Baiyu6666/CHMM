...
def _plot_calls(save_density_plots):
    ...
    model._plot_map_vote_summary = lambda *args, **kwargs: calls.append("mstep")
...
def test_map_density_plots_are_disabled_by_default():
    assert _plot_calls(False) == ["mstep"]


def test_map_density_plots_can_be_enabled():
    assert _plot_calls(True) == ["pooled_density", "demo_density", "mstep"]
