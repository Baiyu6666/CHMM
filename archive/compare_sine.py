...
# ------------------------------------------------------------
# 实验：在 SineCorridorEnv3D 上比较 SegCons 在不同初始化 τ 方案下的敏感性
#
# - Env:   SineCorridorEnv3D（正弦走廊 + 两阶段速度约束）
# - Demo:  env.generate_demos(...)
# - Model: SegCons（配置参考 main_sine_auto.py）
#
# - 两种共享初始化方案：
...
#       * 用这组 taus_init 跑一次 SegCons（tau_init=taus_init, g1_init/g2_init="from_tau"）
# - 记录：
#       loglik
#       tau MAE / NMAE
...
# ------------------------------------------------------------
# 单次：SegCons 在一组 taus_init 下训练 + 提取 metrics
# ------------------------------------------------------------
def run_single_goal(env, demos, true_taus, taus_init, max_iter, seed):
    set_seed(seed)

    # ============= main_sine_auto 里的配置 =============
    # optional learnable feature，如果你要打开，可以取消注释并在 SegCons 里加 learned_features 参数
    # learned_features = [
...
# ------------------------------------------------------------
# 主实验：两种 shared init × SegCons，多次重复
# ------------------------------------------------------------
