...
def _draw_avg_constraint_cost_matrix(ax, learner, demo_idx=0):
    ...
    if not stage_params_list:
        ax.axis("off")
        return
    matrix = np.zeros((learner.num_features, learner.num_states), dtype=float)
    for stage_idx in range(learner.num_states):
        ...
        for k in range(learner.num_states):
            ...
            if learner._is_equality_feature(m):
                ...
                if low is None or high is None:
                    continue
                if not standardized:
                    low = low * learner.feat_std[fid] + learner.feat_mean[fid]
                    high = high * learner.feat_std[fid] + learner.feat_mean[fid]
            ax.fill_between(
                t_seg,
                low,
                high,
...
def _draw_activation_rate_history(ax, learner):
    ...
    num_iters, num_states, num_features = hist.shape
    row_labels = []
    rows = []
...
def _summary_stage_labels(learner):
    return [f"s{i + 1}" for i in range(int(getattr(learner, "num_states", 0)))]
    ...
    if learner.num_states != 2:
        ...
        for stage_idx, (s, e) in enumerate(zip(starts, ends)):
            info = learner._segment_stage_cost_info(
                ...
                shared_stage_subgoals=[np.zeros_like(np.asarray(learner.stage_subgoals[k], dtype=float)) for k in range(learner.num_states)],
                shared_param_vectors=[[None for _ in range(learner.num_features)] for _ in range(learner.num_states)],
                shared_r_mean=None,
...
def _draw_cutpoint_evolution(ax, learner):
    ...
    num_cutpoints = max(int(getattr(learner, "num_states", 1)) - 1, 0)
...
def _scan_cutpoint_range(learner, T, fixed_cutpoints, vary_index):
    ...
    num_cutpoints = int(learner.num_states) - 1
...
def _plot_cutpoint_feature_distribution_compare(learner, it, demo_idx=0, vary_index=1):
    if plt is None or learner.num_states < 3:
        return
    true_cutpoints = _true_cutpoints_for_demo(learner, demo_idx)
    n_cutpoints = learner.num_states - 1
...
def plot_scdp_results_4panel(learner, it, demo_idx=0):
    if plt is None:
        return
    if _is_press_slide_insert(learner.env) and learner.num_states == 4:
    ...
    for vary_index in range(max(int(learner.num_states) - 1, 0)):
        _plot_cutpoint_feature_distribution_compare(learner, it, demo_idx=demo_idx, vary_index=vary_index)


def plot_scdp_results_4panel_overview(learner, it, *, metrics=None, plot_dir=None):
    if plt is None:
        return
    feature_indices = []
